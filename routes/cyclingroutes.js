...
router.get('/', catchAsync(async (req, res) => {
    ...
    res.render('cyclingroutes/index', { campgrounds })
}));

router.get('/new', isLoggedIn, (req, res) => {
    res.render('cyclingroutes/new');
});
...
router.post('/', isLoggedIn, upload.array('image'), validateCampground, catchAsync(async (req, res, next) => {
    ...
    //connect this new cycling routes with the loggedin user
    ...
    req.flash('success', 'Successfully made a new cycling routes!')
    res.redirect(`/cyclingroutes/${campground._id}`)
}))
...
router.get('/:id', catchAsync(async (req, res) => {
    const campground = await Campground.findById(req.params.id)
        ...
        .populate('author');
    if (!campground) {
        req.flash('error', 'Cannot find the cycling routes!')
        return res.redirect('/cyclingroutes');
    }
    res.render('cyclingroutes/show', { campground })
}));
...
router.get('/:id/edit', isLoggedIn, isAuthor, catchAsync(async (req, res) => {
    ...
    res.render('cyclingroutes/edit', { campground })
}));
...
router.put('/:id', isLoggedIn, isAuthor, upload.array('image'), validateCampground, catchAsync(async (req, res) => {
    ...
    req.flash('success', 'Successfully updated cycling routes!')
    res.redirect(`/cyclingroutes/${campground._id}`)
}));

router.delete('/:id', isLoggedIn, isAuthor, catchAsync(async (req, res) => {
    await Campground.findByIdAndDelete(req.params.id);
    req.flash('success', 'Successfully deleted cycling routes!')
    res.redirect('/cyclingroutes')
}));
